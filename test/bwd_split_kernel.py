...
from fwd_kernel import dropout_mask, dropout_offsets, dropout_rng
...
def bwd_kernel_dk_dv(
    ...
    for start_n in range(lo, hi, BLOCK_M):
        ...
        if BLOCK_M == 1:
            ...
            dk += tl.dot(
                tl.trans(ds.to(Q.dtype.element_ty)), q
            )  # (BLOCK_N, BLOCK_DMODEL)
        # update pointers
        Q_block_ptr = tl.advance(Q_block_ptr, (BLOCK_M, 0))
        DO_block_ptr = tl.advance(
            DO_block_ptr, (BLOCK_M, 0)
        )  # Debug DO accessing problems
...
def bwd_kernel_dq(
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if BLOCK_M == 1:
            ...
            dq += tl.dot(
                ds.to(Q.type.element_ty), tl.trans(kt)
