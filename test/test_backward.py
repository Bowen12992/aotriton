...
import pytest
import torch
from _common_test import SdpaContext, SdpaContextFromNPZ, SdpaParams
...
def _do_test_op_bwd(
    ...
    ext = AttentionExtraArgs(
        return_encoded_softmax=True, autotune=False, return_autotune=False
    )
    ...
    is_allclose, adiff, grads_allclose, grads_adiff, tfts = ctx.validate_with_reference(
        tri_out, ctx.dout_tensors, return_target_fudge_factors=True
    )
    ctx.display_validation_results(
        tri_out, is_allclose, adiff, grads_allclose, grads_adiff
    )
    ...
    ), f"{dk_allclose=} {dv_allclose=} {dq_allclose=} {db_allclose=} {tfts=}"
    print(f"{tri_out=}")
...
@pytest.mark.parametrize("storage_flip", [False, True])
# @pytest.mark.parametrize('return_encoded_softmax', [False])
def test_op_bwd_with_matrix_bias(
...
def test_large_bf16_nan_values():
    ...
    dropout_p = 0
    ext = AttentionExtraArgs(
        return_encoded_softmax=causal, autotune=False, return_autotune=False
    )
    tri_out, encoded_softmax, _ = attention(
        q, k, v, b, causal, sm_scale, dropout_p, ext
    )
