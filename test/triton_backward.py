...
def _make_block_eyes(q, base=1.0, inc=0.0):
    ...
    for i in range(0, seqlen, dhead):
        q[:, :, i : i + dhead, :] = (
            torch.eye(dhead, device=q.device, dtype=q.dtype) * scale
        )
        scale += inc
...
def test_op_fwd(
    ...
    q = torch.empty(qdims, dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    k = torch.empty(kdims, dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    v = torch.empty(vdims, dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5)
    if storage_flip:
    ...
    tri_out, encoded_softmax, _ = attention(
        q, k, v, causal, sm_scale, dropout_p, True, autotune
    )
    ...
    if not dv_allclose:
        ...
        err_idx = np.unravel_index(
            torch.argmax(torch.abs(ref_dv - tri_dv)).cpu().numpy(), ref_dv.shape
        )
    ...
    if dv_allclose and not dk_allclose:
        ...
        err_idx = np.unravel_index(
            torch.argmax(torch.abs(ref_dk - tri_dk)).cpu().numpy(), ref_dk.shape
        )
        print(f"{err_idx=}")
        print(
    ...
    if dk_allclose and dv_allclose and not dq_allclose:
        ...
        err_idx = np.unravel_index(
            torch.argmax(torch.abs(ref_dq - tri_dq)).cpu().numpy(), ref_dq.shape
        )
        print(f"{err_idx=}")
        print(
    ...
    assert (
        dk_allclose and dv_allclose and dq_allclose
    ), f"{dk_allclose=} {dv_allclose=} {dq_allclose=}"
