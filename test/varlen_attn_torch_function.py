...
import numpy as np
import torch
from aotriton_flash import attn_bwd_compact_varlen, attn_fwd_compact_varlen
...
class _attention_varlen(torch.autograd.Function):
    ...
    def forward(
        ...
        cu_seqlens_q = torch.tensor(
            [0] + np.cumsum(seqlens_q).tolist(), dtype=torch.int32, device=q.device
        )
        cu_seqlens_k = torch.tensor(
            [0] + np.cumsum(seqlens_k).tolist(), dtype=torch.int32, device=q.device
        )
