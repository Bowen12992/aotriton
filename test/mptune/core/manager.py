...
class TunerManager(ArgArchVerbose):
    ...
    def build_graph(self):
        ...
        dba = self.factory_dbaccessor(
            num_workers=num_workers, side_channel=side_channel
        )
        workers = [
            self.factory_worker(i, gpu_device, side_channel=side_channel)
            for i, gpu_device in enumerate(self._torch_gpus)
        ]
    ...
    def detect_gpu_freeze(self):
        ...
        for monad in self._workers:
            ...
            if lapsed > 5.0:
                monad.kill()
                self.print(
                    f"Kill {monad.identifier} due to inactivity. {status=} {now=} {lapsed=}"
                )
    ...
    def run_watchdog(self):
        ...
        for monad in monads:
            if monad.exitcode != 0:
                ...
                self._state_tracker.update_ui(
                    last_known_working.clone()
                    .set_action(MonadAction.OOB_Died)
                    .update_payload(exitcode=monad.exitcode)
                )
