...
class Monad(ArgArchVerbose):
    ...
    def restart_with_last_progress(self, continue_from):
        ...
        def restart():
            self.print(
                f"Restart {self.identifier} with continue_from = {continue_from}"
            )
            self._process = Process(
                target=self.main, args=(self._q_up, self._q_down, continue_from)
            )
            self._process.start()

        if self._side_channel:
            self._side_channel.put(
                MonadMessage(
                    task_id=None,
    ...
    def main(self, q_up, q_down, continue_from):
        ...
        if self._side_channel:
            self._side_channel.put(
                MonadMessage(
                    task_id=None, action=MonadAction.OOB_Init, source=self.identifier
                )
            )
        ...
        if self._side_channel:
            self._side_channel.put(
                MonadMessage(
                    task_id=None, action=MonadAction.Exit, source=self.identifier
                )
            )
            ...
            try:
                ...
                for res in service.process(req):
                    ...
                    if action == MonadAction.Exit:
                        leaving = True
                        self.print(
                            f"Monad {self.identifier} found one Exit response. Will leave main_loop after forwarding all responses"
                        )
