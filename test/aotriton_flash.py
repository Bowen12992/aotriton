...
from pyaotriton import T1, T2, T4, DType, Stream, get_name_suffix, hipError_t
from pyaotriton.v2.flash import BwdExtraArguments, FwdExtraArguments
from pyaotriton.v2.flash import attn_bwd as fa_backward
from pyaotriton.v2.flash import attn_bwd_compact_varlen as fa_backward_compact_varlen
from pyaotriton.v2.flash import attn_fwd as fa_forward
from pyaotriton.v2.flash import attn_fwd_compact_varlen as fa_forward_compact_varlen
from pyaotriton.v2.flash import debug_fill_dropout_rng as fa_debug_fill_dropout_rng

assert get_name_suffix() != "", (
    "To run tests, AOTriton must be compiled with suffixes "
...
def debug_fill_dropout_rng(R, philox_seed, philox_offset):
    err = fa_debug_fill_dropout_rng(
        mk_aotensor(R), philox_seed, philox_offset, Stream()
    )
    # print(f'debug_fill_dropout_rng {err=}')
    return err


def attn_fwd_compact_varlen(
    q,
    k,
    v,
    cu_seqlens_q,
...
def attn_bwd_compact_varlen(
    ...
    b = mk_aotensor(b, if_empty_then_like=q)
    # print(f'{b=}')
    err = fa_backward_compact_varlen(
        mk_aotensor(q),
        mk_aotensor(k),
        mk_aotensor(v),
        mk_aotensor(cu_seqlens_q),
