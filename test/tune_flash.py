...
from rocm_arch import rocm_get_allarch, rocm_get_gpuarch
...
class FlashTunerSource(MonadService):
        ...
        with open(a.entry_from_json) as fin:
            for line in fin:
                ...
                for (
                    ...
                    a.batch, a.n_heads, a.sm_scale, a.return_encoded_softmax
                ):
    ...
    def process(self, _):
        ...
        continue_dict = {}
        if (
            a.continue_from_json_file
            and a.json_file is not None
            and a.json_file.is_file()
        ):
            # TODO: skipset
            with open(a.json_file, "r") as f:
                for line in f.readlines():
                    ...
                    if (
                        skip is not None
                    ):  # Must test is None, otherwise skip=False when task_id=0
                        skip_set.add(skip)
        ...
        for i, tup in enumerate(self.gen()):
            ...
            if a.selective_set:
                if i in a.selective_set:
                    ...
                    progress_in_db = MonadMessage(
                        task_id=i,
                        action=MonadAction.Pass,
                        source="source",
                        payload=payload,
                    )
            ...
            progress_in_db = MonadMessage(
                task_id=i, action=MonadAction.Pass, source="source", payload=payload
            )
            if i in continue_dict:
...
class FlashTunerManager(TunerManager):
    ...
    def factory_source(self, side_channel):
        return FlashSourceMonad(
            self._args, identifier="source", side_channel=side_channel
        )

    def factory_dbaccessor(self, num_workers, side_channel):
        return mptune.flash.DbMonad(
            self._args,
    ...
    def factory_worker(self, nth_worker: int, gpu_device: int, side_channel):
        return mptune.flash.TunerMonad(
            self._args,
    ...
    def factory_ui(self, state_tracker, src, workers, dbaccessor):
        return mptune.tui.TunerApp(
            state_tracker.get_ui_update_queue(), src, workers, dbaccessor
        )
...
def parse():
    ...
    p.add_argument(
        "--batch", type=int, nargs=1, default=[8], help="(Not a functional) Batch size."
    )
    p.add_argument(
        "--n_heads",
        type=int,
        nargs=1,
        default=[12],
        help="(Not a functional) Number of heads",
    )
    p.add_argument(
        "--sm_scale",
        type=float,
        nargs=1,
        default=[1.2],
        help="(Not a functional) Softmax Scale",
    )
    p.add_argument(
        "--return_encoded_softmax",
        type=bool,
    ...
    p.add_argument(
        ...
        type=int,
        default=8192,
        help="A neat way to limit max value of --seqlen_q.",
    )
    p.add_argument(
        "--max_seqlen_k",
        type=int,
        default=8192,
        help="A neat way to limit max value of --seqlen_k.",
    )
    p.add_argument(
        "--complement_seqlens",
        action="store_true",
        help="Select NOT (seqlen_q <= max_seqlen_q and seqlen_k <= max_seqlen_k)",
    )
    p.add_argument(
        "--limit_memory_at_seqlen",
        type=int,
        default=4096,
        help="When testing with --entry_from_json, use batch=2 and n_heads=2 when seqlen_q*seqlen_k >= this_value ** 2.",
    )
    p.add_argument(
        "--causal",
        type=int,
        ...
        default=[True, False],
        choices=[0, 1],
        help="Causal mask. (Use 0/1 for False/True",
    )
    p.add_argument(
        "--causal_non_square_only",
        action="store_true",
        help="Skip causal=True and seqlen_q == seqlen_k cases.",
    )
    p.add_argument(
        "--dropout_p",
        type=float,
        nargs=NARG_PLUS,
        default=[0.5, 0.0],
        help="Probablity to dropout (0 to disable).",
    )
    p.add_argument(
        "--dtype",
        type=str,
    ...
    p.add_argument(
        ...
        default=[0, 1],
        choices=[0, 1],
        help="Bias types to profile, 0: None, 1: Matrix.",
    )
    p.add_argument("--verbose", action="store_true", help="Verbose")
    p.add_argument(
        "--validate",
        action="store_true",
        help="Validate the correctness of the output to avoid faulty autotune configs",
    )
    p.add_argument(
        "--dry_run",
        action="store_true",
        help="Print parameter combinations without running tests",
    )
    p.add_argument(
        "--continue_from",
        type=int,
        default=None,
        help="Continue from n-th functional set",
    )
    p.add_argument(
        "--stop_at", type=int, default=None, help="Stop at n-th functional set"
    )
    p.add_argument(
        "--selective_set",
        type=int,
        default=[],
        nargs="*",
        help="Only use the given task ids. Will override other options like --continue_from",
    )
    p.add_argument(
        "--sc_report",
        type=str,
        default=None,
        help="Re-run tests according to sc_report file generated by `table_tool --action rawsc`",
    )
    p.add_argument(
        "--entry_from_json",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Sqlite Database file (not recommended)",
    )
    p.add_argument(
        "--json_file",
        type=Path,
    ...
    p.add_argument(
        "--overwrite_json_file",
        dest="continue_from_json_file",
        action="store_false",
        help='Do NOT "Append to Json file instead of overwrite, and skip already tested entries."',
    )
    p.add_argument(
        "--create_table_only",
        action="store_true",
        help="Do not insert data, only create tables. Used for schema updates.",
    )
    p.add_argument(
        "--use_multigpu",
        type=int,
        nargs="+",
        default=None,
        help="Profiling on multiple GPUs. Passing -1 for all GPUs available to pytorch.",
    )
    p.add_argument(
        "--arch",
        type=str,
        default=None,
        help="[NOT RECOMMENDED TO SET MANUALLY] Override GPU architecture string. Will use first GPU from `rocm_agent_enumerator -name` if not provided.",
    )
    p.add_argument(
        "--confirm_to_override_arch",
        action="store_true",
        help="A defensive option to avoid setting --arch unintentionally.",
    )
    p.add_argument(
        "--debug_skip_next_kernel",
        type=int,
        default=[],
        nargs="*",
    ...
    p.add_argument(
        "--debug_headless",
        action="store_true",
        help="Set headless mode for textual.App.",
    )
    args = p.parse_args()
    assert args.return_encoded_softmax == [False], (
        "Do not support tuning return_encoded_softmax=True. "
    ...
    if args.entry_from_json:
        if not args.entry_from_json.endswith(".cfg"):
            print(
                "--entry_from_json should only take files with .cfg suffix, to avoid potential conflict with --json_file"
            )
            exit(-1)
...
def main():
    assert (
        os.getenv("PYTORCH_NO_CUDA_MEMORY_CACHING", default=0) == 0
    ), "PYTORCH_NO_HIP_MEMORY_CACHING does not play nicely with torch.multiprocessing. See https://github.com/pytorch/pytorch/issues/114534"
    # We tried
