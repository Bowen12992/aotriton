...
def _do_test_op_fwd(
    ...
    is_allclose, adiff, _, _ = ctx.validate_with_reference(
        tri_out, None, no_backward=True
    )
    if not is_allclose:
        import numpy as np

        err_idx = np.unravel_index(
            torch.argmax(torch.abs(ref_out.to(device=tri_out.device) - tri_out))
            .cpu()
            .numpy(),
            ref_out.shape,
        )
