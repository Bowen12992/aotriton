...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if return_encoded_softmax:
            encoded_softmax = torch.zeros(
                ...
                device=q.device,
                dtype=_attention.DEBUG_MASK_DTYPE,
            )
        else:
    ...
    def backward(ctx, do, _, __):
        ...
        assert (
            q.stride() == do.stride()
        )  # Hard requirement, hard-coded in the triton kernel
        print(f"{o.stride()=}")
        print(f"{do.stride()=}")
        ...
        BLOCK = (
            BLOCK
            if is_supported_by_tl_dot(seqlen_q) and is_supported_by_tl_dot(seqlen_k)
            else 1
        )
        ...
        if q.requires_grad:
            ...
            bwd_kernel_dq[
                (triton.cdiv(q.shape[2], DQ_BLOCK_M), q.shape[1], q.shape[0])
            ](
                q,
                k,
                v,
