...
class KernelTuningDatabase(object):
    ...
    def __init__(
        ...
        if self._build_for_tuning:
            return
        td = (
            pathlib.Path(tune_info_dir) / self.MONOLITHIC_TUNING_DATABASE_FILE
        )  # in case tune_info_dir is str
        ...
        for (arch,) in res.fetchall():
            dba = SQLiteKernelTuningDatabaseForArch(
                k, arch, self._conn, table_name, downgrader
            )
    ...
    def select_gpu(self, gpu, index):
        ...
        if arch not in self.arch_dict:
            if not self._build_for_tuning:
                print(
                    f"For kernel {self._kdesc.KERNEL_FAMILY}.{self._kdesc.name}, Architecture {arch} was not found in tuning database, using dummy one instead"
                )
                self.arch_dict[arch] = EmptyKernelTuningDatabaseForArch(
                    self._kdesc, arch
                )
            else:
