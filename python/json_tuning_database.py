...
class JsonKernelTuningDatabaseForArch(CommonKernelTuningDatabaseForArch):
    ...
    def _build_db_index(self, fsels):
        ...
        for ti in self._j["tune_info"]:
            ...
            for eti in self._index_dedup[tup]:
                # print(f'{eti=}')
                if (
                    eti["tuned_kernel"] == ti["tuned_kernel"]
                    and eti["compiler_options"] == ti["compiler_options"]
                ):
                    is_dup = True
                    break
            if not is_dup:
    ...
    def _lookup_tuning_info(self, fsels, with_duplicates=True):
        ...
        fallback_tup, fallback_applied_fsels = self._extract_keys_from_fsels(
            fsels, use_fallback_for_partially_tuned=True
        )
        print(
            f"Functionals {tup} cannot be found in tuning db, use {fallback_tup} instead"
        )
        assert fallback_tup in self._index
        tuning_info = (
            self._index[fallback_tup]
            if with_duplicates
            else self._index_dedup[fallback_tup]
        )
    ...
    def get_lut(
        ...
        if tup not in self._lut:
            ...
            # print(f'{tup=}')
            assert indexed
            self._lut[tup] = KernelTuningEntryForFunctionalOnGPU(
                kdesc, self, fsels, indexed, autotune_keys, perf_meta
            )
