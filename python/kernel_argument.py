...
class ArgumentMetadata(object):
    ...
    def __init__(
        self, grouped_arguments_as_set, possible_values, cat: ArgumentCategory, kdesc
    ):
    ...
    def sort_arguments(self, ALL_ARGUMENTS):
        arguments_tuple = [
            (aname, ALL_ARGUMENTS.index(aname))
            for aname in self._grouped_arguments_as_set
        ]
    ...
    def first_apperance(self):
        assert hasattr(
            self, "_first_apperance"
        ), "ArgumentMetadata: must call sort_arguments before first_apperance"
    ...
    def ordered_argument_places(self):
        ...
        ), "ArgumentMetadata: must call sort_arguments before ordered_argument_places"
    ...
    def godel_number(self):
        assert hasattr(
            self, "_godel_number"
        ), "ArgumentMetadata: must call assign_godel_number on all ArgumentMetadata objects before using godel_number property"
    ...
    def param_cc_fields(self):
        ...
        if triton_arg.startswith("stride_"):
            return []
        return [
            self.get_param_cc_type(a[0]) + " " + a[0] for a in self._ordered_arguments
        ]
    ...
    def codegen_godel_number_calculation(self, fout):
        ...
        if self.is_tensor:
            for number, possible_type in enumerate(self._possible_values):
                ...
                print(
                    2 * INDENT
                    + f"if ({triton_arg}->dtype() == {self.DTYPE_NUMBER[elem_type]}) number = {number} ;",
                    file=fout,
                )
        else:
            for number, possible_type in enumerate(self._possible_values):
                ...
                print(
                    2 * INDENT + f"if ({triton_arg} == {value}) number = {number} ;",
                    file=fout,
                )
