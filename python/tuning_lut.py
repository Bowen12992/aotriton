...
class KernelTuningEntryForFunctionalOnGPU(object):
    ...
    def __init__(
        ...
        self._autotune_key_values = (
            {key: set() for key, _ in autotune_keys}
            if autotune_keys is not None
            else None
        )
        self._autotune_key_class = (
            {key: klass for key, klass in autotune_keys}
            if autotune_keys is not None
            else None
        )
        self._sigs = []
        ...
        if autotune_keys is None:
            ...
            if dba.is_passthrough_tuning():
                ...
                for row in rows:
                    psels, compiler_options = dba.craft_perf_selection(
                        columns, row, perf_meta
                    )
                    self._allocate_sig(psels, compiler_options)
                self._lut_dic[0] = 0
            else:
                default_psels, default_co = dba.craft_perf_selection(
                    None, None, perf_meta
                )
        ...
        for row in rows:
            ...
            self._lut_dic[fs_atk_values] = self._allocate_sig(psels, compiler_options)[
                0
            ]
        assert self._sigs
    ...
    def extract_autotune_key_values(self, columns, row):
        ...
        return tuple(
            [
                self.track_autotune_key_values(columns, row, tup)
                for tup in self._autotune_keys
            ]
        )
    ...
    def _allocate_sig(self, psels, compiler_options):
        sig = KernelSignature(
            self._kdesc, self._fsels, psels, compiler_options, self._dba.gpu
        )
    ...
    def _build_lut_tensor(self):
        self._autotune_key_buckets = [
            klass(self._autotune_key_values[key]) for key, klass in self._autotune_keys
        ]
        ...
        self._lut_tensor = np.empty(
            [bucket.nvalues for bucket in self._autotune_key_buckets], dtype=dtype
        )
        assert (
            self._lut_tensor.size > 0
        ), "LUT tensor must be non-empty. Empty LUT is not constructed by _build_lut_tensor"
        self._lut_cshape = "".join([f"[{s}]" for s in self._lut_tensor.shape])
        self._list_of_atk_representatives = [
            bucket.representatives for bucket in self._autotune_key_buckets
        ]
        list_of_atk_indices = [
            range(bucket.nvalues) for bucket in self._autotune_key_buckets
        ]
        for indices, atk_values in zip(
            itertools.product(*list_of_atk_indices),
    ...
    def codegen_package_path(self, kernel_image_dir):
        for _, _, o in self.gen_kernel_symbols(kernel_image_dir):
            ...
            return (
                'R"xyzw('
                + str(dir_arch / o.KERNEL_FAMILY / o.SHIM_KERNEL_NAME / fonly)
                + ')xyzw"'
            )
    ...
    def codegen_kernel_image_objects(self, kernel_image_dir, noimage_mode):
        ...
        for _, _, o in self.gen_kernel_symbols(kernel_image_dir):
            if not noimage_mode:
                assert (
                    o.compiled_files_exist
                ), f"Compiled file {o._hsaco_kernel_path} not exists"
    ...
    def write_lut_source(
        self, library_suffix: str, outdir: "pathlib.Path", bare_mode, noimage_mode
    ):
        gpu_kernel_image_dir = (
            outdir.parent / f"gpu_kernel_image.{self._kdesc.SHIM_KERNEL_NAME}"
        )
        ...
        d = {
            ...
            "kernel_image_objects": self.codegen_kernel_image_objects(
                gpu_kernel_image_dir, noimage_mode=noimage_mode
            ),
        ...
        if raise_lut_entry:
            raise MissingLutEntry(
                self._dba.gpu, self._kdesc, ofn, self._fsels, lut_tensor
            )
        return ofn
    ...
    def lut_cdata(self):
        ...
        return (
            np.array2string(lut_tensor, separator=",")
            .replace("[", "{")
            .replace("]", "}")
        )
        # cdata = io.StringIO()
    ...
    def codegen_binned_indices(self):
        if self._untuned:
            return "[0]"
        return "".join(
            [f"[{key}{self.BIN_INDEX_SUFFIX}]" for key, _ in self._autotune_keys]
        )
