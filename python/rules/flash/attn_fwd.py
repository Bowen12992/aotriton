...
class attn_fwd(FlashKernel):
    ...
    TYPE_CHOICES = {
        frozenset(
            ["Q", "K", "V", "B", "Out", "encoded_softmax"]
        ): FlashKernel.MAIN_DATATYPES,
        frozenset(["sm_scale"]): ["fp32"],
        frozenset(["M"]): ["*fp32:16"],
        frozenset(["cu_seqlens_q", "cu_seqlens_k"]): ["*i32:16"],
        frozenset(
            ["num_head_q", "num_head_k", "num_seqlens", "max_seqlen_q", "max_seqlen_k"]
        ): ["i32"],
        frozenset(["head_dim"]): ["i32"],
        frozenset(["dropout_p"]): ["fp32"],
        frozenset(["philox_seed_ptr", "philox_seed_output", "philox_offset_output"]): [
            "*u64"
        ],
    ...
    def gen_autotune_configs(gpu: str, fsel_dict: "dict[str, Any]"):
        ...
        NUM_STAGES = [1, 2]
        for (M, N), waves, warps, stages, pre in itertools.product(
            BLOCK_SIZES, WAVES_PER_EU, NUM_WARPS, NUM_STAGES, PRE_LOAD_V
        ):
