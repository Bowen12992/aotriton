...
class bwd_kernel_dq(FlashKernel):
    ...
    TYPE_CHOICES = {
        ...
        frozenset(["num_seqlens", "max_seqlen_q", "max_seqlen_k"]): match_fwd(
            "num_seqlens"
        ),
    ...
    def gen_autotune_configs(gpu, fsel_dict: "dict[str, Any]"):
        ...
        NUM_WARPS = [1, 2, 4]
        NUM_STAGES = [1, 2]
        for M, N, waves, warps, stages in itertools.product(
            BLOCK_SIZES, BLOCK_SIZES, WAVES_PER_EU, NUM_WARPS, NUM_STAGES
        ):
            if M < N:
