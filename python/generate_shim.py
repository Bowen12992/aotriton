...
def parse():
    ...
    p.add_argument(
        "--archive_only",
        action="store_true",
        help="Only generate archive library instead of shared library. No linking with dependencies.",
    )
    p.add_argument(
        "--library_suffix",
        type=str,
        default="",
        help="Add suffix to the library name 'aotriton' to avoid symbol conflicts",
    )
    p.add_argument(
        "--bare_mode",
        action="store_true",
        help="Instead of generating a proper Makefile, only generate a list of source files and leave the remaining tasks to cmake.",
    )
    p.add_argument(
        "--noimage_mode",
        action="store_true",
        help="Expect the GPU kernel images are built separately.",
    )
    p.add_argument(
        "--build_for_tuning",
        action="store_true",
        help="Include all GPU kernels in the dispatcher for performance tuning.",
    )
    ...
    def write_body(self):
        for cfn in self._srcdir.rglob("*.cc"):
            ...
            if self.is_bare:
                ...
                cmd = (
                    self._cc_cmd
                    + f" {cfn.absolute()} -I{self._build_dir.absolute()} -o {ofn.absolute()} -c"
                )
                print("\t", cmd, "\n", file=self._out)
...
class KernelShimGenerator(MakefileSegmentGenerator):
    ...
    def __init__(self, args, out, k: "KernelDescription"):
        ...
        self._shim_hdr = NoWriteIfNoUpdateFile(
            self._shim_path / Path(self.SHIM_FILE_STEM + ".h")
        )
        ...
        # Autotune dispatcher
        self._autotune_path = (
            Path(args.build_dir) / k.KERNEL_FAMILY / f"autotune.{k.SHIM_KERNEL_NAME}"
        )
        self._autotune_path.mkdir(parents=True, exist_ok=True)
        self._ktd = KernelTuningDatabase(
            SOURCE_PATH.parent / "rules",
            k,
    ...
    def write_body(self):
        ...
        if self.is_bare:
            ...
            print(
                makefile_target,
                ":",
                str(self._shim_hdr.path.absolute()),
                str(self._shim_src.path.absolute()),
                file=self._out,
            )
            cmd = (
                self._cc_cmd
                + f" {self._shim_src.path.absolute()} -o {ofn.absolute()} -c -fPIC -std=c++20"
            )
            print("\t", cmd, "\n", file=self._out)
        self._objpaths.append(makefile_target)
    ...
    def gen_children(self, out):
        ...
        for gpu, fsels, lut in k.gen_tuned_kernel_lut(self._ktd):
            ...
            yield AutotuneCodeGenerator(
                args, self.children_out, self._autotune_path, k, gpu, fsels, lut
            )
            """
            debug_counter +=1
            if debug_counter >= 2:
        ...
        if self.is_bare:
            return
        for o in k.gen_all_object_files(
            p,
            tuned_db=self._ktd,
            sancheck_fileexists=not args.build_for_tuning and not args.noimage_mode,
        ):
    ...
    def write_conclude(self):
        if self.is_bare:
            return
        objs = [
            c._odesc for c in self._children if isinstance(c, ObjectShimCodeGenerator)
        ]
        self._kdesc.write_shim_header(self._fhdr, objs)
        self._kdesc.write_shim_source(
            self._fsrc, objs, noimage_mode=self._args.noimage_mode
        )
...
class AutotuneCodeGenerator(MakefileSegmentGenerator):
    ...
    def write_body(self):
        ...
        try:
            self._ofn = self._lut.write_lut_source(
                self._args.library_suffix,
                self._outdir,
        ...
        if self.is_bare:
            ...
            print(
                ...
                ":",
                self._ofn.relative_to(self._build_dir),
                file=self._out,
            )
            cmd = (
                self._cc_cmd
                + f" {self._ofn.absolute()} -o {self._obj_fn.absolute()} -c"
            )
            print("\t", cmd, "\n", file=self._out)
        if do_raise:
            raise do_raise
