...
def sdpa_math(
    ...
    if torch.__version__ >= "2.5.0":
        ...
        retv = torch.ops.aten._scaled_dot_product_attention_math(
            query,
            key,
            value,
            dropout_p=dropout_p,
...
SdpaParams = namedtuple(
    "SdpaParams", ["causal", "sm_scale", "dropout_p", "dropout_mask"]
)
...
class SdpaContext(object):
    ...
    def clone_tensor_tuple(in_tensors, dtype, device=None):
        return tuple(
            [
                SdpaContext.clone_tensor(t, dtype=dtype, device=device)
                for t in in_tensors
            ]
        )
    ...
    def create_ref_inputs(self, target_gpu_device="cuda"):
        ...
        if ref_device_option == "default":
            ...
            assert (
                False
            ), f'Unknown ref_device_option value {ref_device_option}. Allowed choices "default" "cpu" "cuda"'
    ...
    def create_ref_inputs_with_device(self, ref_device):
        ...
        self.ref_tensors = self.clone_tensor_tuple(
            self.dev_tensors, dtype=hp_dtype, device=ref_device
        )
        self.lp_ref_tensors = self.clone_tensor_tuple(
            self.dev_tensors, dtype=dtype, device=ref_device
        )
    ...
    def set_require_grads(self, skip_dq=False, skip_dk_dv=False, skip_db=False):
        self._require_grads(
            self.dev_tensors, skip_dq=skip_dq, skip_dk_dv=skip_dk_dv, skip_db=skip_db
        )
        self._require_grads(
            self.ref_tensors, skip_dq=skip_dq, skip_dk_dv=skip_dk_dv, skip_db=skip_db
        )
        self._require_grads(
            self.lp_ref_tensors, skip_dq=skip_dq, skip_dk_dv=skip_dk_dv, skip_db=skip_db
        )
    ...
    def _compute_ref_forward(ref_tensors, p: SdpaParams):
        ...
        dropout_mask = (
            p.dropout_mask
            if p.dropout_mask is None
            else p.dropout_mask.to(device=ref_q.device)
        )
        # _scaled_dot_product_attention_math seems also working for nested tensor
        ref_out, ref_mask = sdpa_math(
            ref_q,
            ref_k,
            ref_v,
    ...
    def compute_backward(self, out, dout, *, ref_only=False):
        self.dref_tensors = self._compute_backward(
            self.ref_tensors, self.refout_tensors[0], dout
        )
        self.lp_dref_tensors = self._compute_backward(
            self.lp_ref_tensors, self.lp_refout_tensors[0], dout
        )
        if not ref_only:
    ...
    @staticmethod
    def _validate(
        out, ref, lp_ref, fudge_factor, tname, *, return_target_fudge_factors=False
    ):
        ...
        if math.isnan(test_error) and not math.isnan(ref_error):
            ...
            f"Tensor {tname} has NaN output but not NaN reference"
    ...
    def validate_with_reference(
        ...
        *,
        ...
        if no_forward:
            ...
            out_allclose, out_adiff, tft = self._validate(
                out,
        ...
        for grad, ref, lp_ref, fudge_factor, tname in zip(
            grads,
            self.dref_tensors,
            self.lp_dref_tensors,
            self.fudge_factors,
            self.TENSOR_NAMES,
        ):
            allclose, adiff, tft = self._validate(
                grad,
                ref,
                lp_ref,
                fudge_factor,
        ...
        if return_target_fudge_factors:
            return (
                ...
                grads_allclose,
                grads_adiff,
                target_fudge_factors,
            )
        else:
            return out_allclose, out_adiff, grads_allclose, grads_adiff

    def display_validation_results(
        self, tri_out, is_allclose, adiff, grads_allclose, grads_adiff
    ):
        q, k, v, b = self.dev_tensors
        ...
        if not is_allclose:
            err_idx = np.unravel_index(
                torch.argmax(torch.abs(TO(ref_out) - tri_out)).cpu().numpy(),
                ref_out.shape,
            )
            print(f"{err_idx=}")
        ...
        if not dv_allclose:
            err_idx = np.unravel_index(
                torch.argmax(torch.abs(TO(ref_dv) - tri_dv)).cpu().numpy(), ref_dv.shape
            )
        ...
        if dv_allclose and not dk_allclose:
            ...
            err_idx = np.unravel_index(
                torch.argmax(torch.abs(TO(ref_dk) - tri_dk)).cpu().numpy(), ref_dk.shape
            )
            print(f"{err_idx=}")
            print(
                f"{tri_dk[err_idx]=} {ref_dk[err_idx]=} error = {torch.abs(tri_dk[err_idx] - ref_dk[err_idx])}"
            )
        ...
        if dk_allclose and dv_allclose and not dq_allclose:
            err_idx = np.unravel_index(
                torch.argmax(torch.abs(TO(ref_dq) - tri_dq)).cpu().numpy(), ref_dq.shape
            )
            print(f"{err_idx=}")
            print(
                f"{tri_dq[err_idx]=} {ref_dq[err_idx]=} error = {torch.abs(tri_dq[err_idx] - ref_dq[err_idx])}"
            )

        if dk_allclose and dv_allclose and dq_allclose and not db_allclose:
            err_idx = np.unravel_index(
                torch.argmax(torch.abs(TO(ref_db) - tri_db)).cpu().numpy(), ref_db.shape
            )
            print(f"{err_idx=}")
            print(
                f"{tri_db[err_idx]=} {ref_db[err_idx]=} error = {torch.abs(tri_db[err_idx] - ref_db[err_idx])}"
            )
...
class VarlenSdpaContext(SdpaContext):
    ...
    def _compute_ref_forward_varlen(ref_tensors, seqlens_q, seqlens_k, p: SdpaParams):
        ...
        packed_dropout_mask = (
            p.dropout_mask
            if p.dropout_mask is None
            else p.dropout_mask.to(device=packed_ref_q.device)
        )
        ref_out_array = []
        ref_mask_array = []
        seqlen_q_start = 0
        ...
        for i, (seqlen_q, seqlen_k) in enumerate(zip(seqlens_q, seqlens_k)):
            ...
            dropout_mask = (
                packed_dropout_mask[i, :, :, :]
                if packed_dropout_mask is not None
                else None
            )
            print(
                f"REF {seqlen_q_start=} {seqlen_q_start+seqlen_q=} {ref_q.shape=} {ref_k.shape=} {ref_v.shape=}"
            )
            ...
            if dropout_mask is not None:
                ...
                dropout_mask = dropout_mask[
                    :, :seqlen_q, :seqlen_k
                ]  # Trim to actual seqlen
                print(f"REF CLAMPED {dropout_mask.shape=}")
                # print(f'REF {dropout_mask=}')
            ref_out, ref_mask = torch.ops.aten._scaled_dot_product_attention_math(
                ref_q,
                ref_k,
                ref_v,
                dropout_p=p.dropout_p,
    ...
    def compute_ref_forward(self, p: SdpaParams):
        self.fudge_factors = self._compute_fudge_factors(p)
        self.refout_tensors = self._compute_ref_forward_varlen(
            self.ref_tensors, self._seqlens_q, self._seqlens_k, p
        )
        self.lp_refout_tensors = self._compute_ref_forward_varlen(
            self.lp_ref_tensors, self._seqlens_q, self._seqlens_k, p
        )
        return self.lp_refout_tensors
...
class SdpaContextFromNPZ(SdpaContext):
    def __init__(self, fn, dtype, device="cuda"):
        ...
        assert not np.isnan(
            sm_scale
        ), "FIXME: suppport scale=None when capturing torch.nn.functional.scaled_dot_product_attention"
        self.sdpa_params = SdpaParams(
    ...
    def compute_backward(self, out, dout, *, ref_only=False):
        assert (
            ref_only == False
        ), "SdpaContextFromNPZ.compute_backward is incompatible with ref_out=True"
        self.dout_tensors = self._compute_backward(self.dev_tensors, out, dout)
