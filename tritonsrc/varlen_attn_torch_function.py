...
from flash import bwd_preprocess_varlen as bare_bwd_preprocess_varlen
...
class _varlen_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        cu_seqlens_q = torch.tensor(
            [0] + np.cumsum(seqlen_q).tolist(), dtype=torch.int32, device=q.device
        )
        cu_seqlens_k = torch.tensor(
            [0] + np.cumsum(seqlen_k).tolist(), dtype=torch.int32, device=q.device
        )
        ...
        M = torch.zeros(
            (batch, num_heads, max_seqlen_q), device=q.device, dtype=torch.float32
        )
        if return_encoded_softmax:
            encoded_softmax = (
                torch.ones(
                    ...
                    device=q.device,
                    dtype=_varlen_attention.DEBUG_MASK_DTYPE,
                )
                * 114.514
            )
        ...
        if autotune:
            ...
            BLOCK_M = BLOCK_N = 16
            print(
                f"{BLOCK_M=} {BLOCK_N=} {RETURN_ENCODED_SOFTMAX=} seqlen_q={seqlen_q} seqlen_k={seqlen_k} {cu_seqlens_q=} {cu_seqlens_k=} {q.shape=} {q.stride()=}",
                flush=True,
            )
            bare_attn_fwd[grid](
                q,
        ...
        ctx.bias_type = 0
        ctx.tuning_result = (
            [("attn_fwd_varlen", tuning_result)] if tuning_result is not None else None
        )
