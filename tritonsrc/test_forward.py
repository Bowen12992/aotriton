...
def scaled_dot_product_attention(
    ...
    print(
        f"BEFORE DROPOUT_MASK {attn_weight[:,:, :SPARSE_SEQ_SINCE, :SPARSE_HEAD_SINCE]=}"
    )
    if dropout_p > 0.0:
        if dropout_mask is not None:
            ...
            print(
                f"AFTER DROPOUT_MASK {attn_weight[:,:, :SPARSE_SEQ_SINCE, :SPARSE_HEAD_SINCE]=}"
            )
...
class FwdTester(object):
    ...
    def do_test_op_fwd(
        ...
        print(
            f"test_op_fwd {BATCH=}, {N_HEADS=}, {seqlen_q=}, {seqlen_k=}, {D_HEAD=}, {causal=}"
        )
        SPARSE_HEAD_SINCE = 3
        SPARSE_SEQ_SINCE = 3
        Z = BATCH
        ...
        if True:  # Real UT
            ...
            if bias_type is None:
                ...
                b = torch.empty(bdims, dtype=dtype, device="cuda").normal_(
                    mean=0.0, std=0.5
                )
            else:
        ...
        if False:
            ...
            q = torch.arange(
                np.prod([Z, H, seqlen_q, D_HEAD]), dtype=dtype, device="cuda"
            ).reshape((Z, H, seqlen_q, D_HEAD))
            k = torch.arange(
                np.prod([Z, H, seqlen_k, D_HEAD]), dtype=dtype, device="cuda"
            ).reshape((Z, H, seqlen_q, D_HEAD))
            v = torch.arange(
                np.prod([Z, H, seqlen_k, D_HEAD]), dtype=dtype, device="cuda"
            ).reshape((Z, H, seqlen_q, D_HEAD))
        ...
        ext = AttentionExtraArgs(
            ...
            and not self.use_fill_rng_for_dropout,
            autotune=False,
        ...
        ref_out, ref_softmax = torch.ops.aten._scaled_dot_product_attention_math(
            q,
            k,
            v,
            dropout_p=dropout_p,
        ...
        if False:
            mref_out, mref_softmax = scaled_dot_product_attention(
                q,
                k,
                v,
                dropout_p=dropout_p,
            ...
            print(
                f"{tri_out[:,:,  :SPARSE_SEQ_SINCE, :SPARSE_HEAD_SINCE]/ref_out[:,:,  :SPARSE_SEQ_SINCE, :SPARSE_HEAD_SINCE]=}"
            )
        ...
        if not is_allclose:
            ...
            print(f"{err_idx=}")
            print(
                f"{tri_out[err_idx]=} {ref_out[err_idx]=} error: {tri_out[err_idx] - ref_out[err_idx]}"
            )
        # if not is_allclose:
        if False:
            import numpy as np
...
@pytest.mark.parametrize("storage_flip", [False, True])
# @pytest.mark.parametrize('return_encoded_softmax', [False])
def test_op_fwd_with_matrix_bias(
    BATCH, N_HEADS, D_HEAD, seqlen_q, seqlen_k, sm_scale, dropout_p, dtype, storage_flip
):
    causal = False
    bias_type = "matrix"
    tester = FwdTester()
