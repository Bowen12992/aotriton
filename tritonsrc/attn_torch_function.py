...
from sized_tuned_bwd import sized_tuned_bwd_kernel_dk_dv, sized_tuned_bwd_kernel_dq
from tuned_bwd import tuned_bwd_kernel_dk_dv, tuned_bwd_kernel_dq
...
TRITON_CONFIG_LIST_FWD = [
    triton.Config(
        {"BLOCK_M": 128, "BLOCK_N": 64, "waves_per_eu": 0, "pre_load_v": True},
        num_stages=1,
        num_warps=4,
    ),
    triton.Config(
        {"BLOCK_M": 128, "BLOCK_N": 64, "waves_per_eu": 1, "pre_load_v": True},
        num_stages=1,
        num_warps=4,
    ),
    triton.Config(
    ...
    triton.Config(
        {"BLOCK_M": 128, "BLOCK_N": 64, "waves_per_eu": 4, "pre_load_v": True},
        num_stages=1,
        num_warps=4,
    ),
    triton.Config(
    ...
    triton.Config(
        {"BLOCK_M": 128, "BLOCK_N": 64, "waves_per_eu": 4, "pre_load_v": False},
        num_stages=1,
        num_warps=4,
    ),
]
                ...
                for waves_per_eu in range(0, 4 + 1):
                    ...
                    for num_stages in [0, 1]:
                        for num_warps in [1, 2, 4, 8]:
                            cfg = triton.Config(
                                dic, num_stages=num_stages, num_warps=num_warps
                            )
...
class _attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if autotune:
            assert (
                False
            ), "tritonsrc based autotune is disabled for now due to potentially faulty triton.Config on Navi31"
            tuned_attn_fwd[grid](
                q,
                k,
                v,
            ...
            RETURN_ENCODED_SOFTMAX = encoded_softmax is not None
            print(
                f"{BLOCK_M=} {BLOCK_N=} {RETURN_ENCODED_SOFTMAX=} seqlen_q={q.shape[2]} seqlen_k={k.shape[2]}",
                flush=True,
            )
            print(
                f"{q.data_ptr()=:x} {k.data_ptr()=:x} {v.data_ptr()=:x} {b.data_ptr()=:x} {M.data_ptr()=:x} {o.data_ptr()=:x}",
                flush=True,
            )
            if RETURN_ENCODED_SOFTMAX:
                print(f"{encoded_softmax.data_ptr()=:x}", flush=True)
            print(
                f"{q.shape=} {k.shape=} {v.shape=} {b.shape=} {M.shape=} {o.shape=}",
                flush=True,
            )
            print(
                f"{q.stride()=} {k.stride()=} {v.stride()=} {b.stride()=} {M.stride()=} {o.stride()=}",
                flush=True,
            )
            bare_attn_fwd[grid](
                q,
        ...
        ctx.tuning_result = (
            [("attn_fwd", tuning_result)] if tuning_result is not None else None
        )
        ctx.attn_extra_args = attn_extra_args
        if ctx.tuning_result is not None:
            for kernel_name, best in ctx.tuning_result:
                print(
                    f"{kernel_name=} {best.kwargs=} {best.num_warps=} {best.num_stages=}"
                )
    ...
    def backward_split(ctx, do, _, fwd_tuning_result):
        ...
        if db.numel() == 0 or not b.requires_grad:
            ...
            db.fill_(float("nan"))
        print(
            f"backward {ctx.bias_type=} {ctx.autotune=} {BLOCK_M=} {BLOCK_N=} {stride_dbz=} {stride_dbh=} {stride_dbm=} {stride_dbn=}"
        )
        if k.requires_grad and v.requires_grad:
            if ctx.autotune:
                ...
                if return_autotune:
                    dkdv_best_config = copy.deepcopy(
                        sized_tuned_bwd_kernel_dk_dv.get_best_config()
                    )
        ...
        if False:
            mask_allclose = torch.allclose(
                torch.abs(debug_mask), torch.abs(ctx.encoded_softmax)
            )  # Stores QK
        ...
        if q.requires_grad:
            if ctx.autotune:
                ...
                if return_autotune:
                    dq_best_config = copy.deepcopy(
                        sized_tuned_bwd_kernel_dq.get_best_config()
                    )
                    # BLOCK_M/N are missing with sized_tuned_bwd_kernel_*
    ...
    def backward_fused(ctx, do, _, fwd_tuning_result):
        ...
        grid = lambda META: (
            max(
                triton.cdiv(max_seqlen_k, META["BLOCK_M1"]),
                triton.cdiv(max_seqlen_q, META["BLOCK_M2"]),
            ),
            num_head_q,
            batch,
        )
...
def debug_fill_dropout_rng(dropout_rng, philox_seed, philox_offset):
    ...
    r = dropout_rng
    bare_debug_fill_dropout_rng[grid_rng](
        r,
        r.stride(0),
        r.stride(1),
        r.stride(2),
